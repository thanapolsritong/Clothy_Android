...
export default function HomeScreen({ navigation }: any) {
  ...
  const handleSave = () => {
    if (name.trim() && phone.trim()) {
      ...
      alert('กรุณากรอกชื่อและเบอร์โทรศัพท์ (ข้อมูลจำเป็น)');
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.listContainer}>
        <Text style={styles.sectionTitle}>รายชื่อลูกค้า</Text>
        <FlatList
          data={customers}
  ...
  onPress={() => Alert.alert(
    'ลบลูกค้า',
    `ต้องการลบ "${item.name}" ออกจากระบบ?\nชุดทั้งหมดของลูกค้าคนนี้จะหายไปด้วย`,
    [
...
>
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              <Feather name="users" size={48} color="#d1d1cc" />
              <Text style={styles.emptyStateText}>ยังไม่มีรายชื่อลูกค้า</Text>
              <Text style={styles.emptyStateSub}>กดปุ่ม + เพื่อเพิ่มข้อมูล</Text>
            </View>
          }
        />
      </View>
...
const styles = StyleSheet.create({
  ...
  statsContainer: { flexDirection: 'row', paddingHorizontal: 20, marginBottom: 20, gap: 15 },
  ...
  emptyStateText: { fontSize: 16, fontWeight: 'bold', color: '#666', marginTop: 15 },
